 ...
 ******************************************/
$light_rule_file_location = "..\\ontology".$output_suffix."\\genomic-cds".$output_suffix.".owl";//Light ontology version with the conceptualization of haplotypes and the CDS and phenotype rules.
$light_rule_demo_file_location = "..\\ontology".$output_suffix."\\genomic-cds_demo".$output_suffix.".owl";//Light ontology version with demo patient's genotype based on direct association of user with SNPs variant classes and the genomic-cds_rules ontology. 

$full_rule_file_location = "..\\ontology".$output_suffix."\\MSC_classes".$output_suffix.".owl";//Full ontology version with the conceptualization of human SNP variant classes, human haplotype classes and the CDS and phenotype rules.
$full_rule_demo_file_location = "..\\ontology".$output_suffix."\\MSC_classes_demo".$output_suffix.".owl";//Full ontology version with a demo patient's genotype based on human SNP variants and the MSC_classes ontology.

$CDS_server_file_location = "..\\ontology".$output_suffix."\\MSC_textual_rules".$output_suffix.".owl";// Full ontology version but with only the textual description of CDS and phenotype rules. It is the one used in MSC server.
$CDS_server_demo_file_location = "..\\ontology".$output_suffix."\\MSC_textual_rules_demo".$output_suffix.".owl";// Full ontology version with a demo patient's genotype based on human SNP variants and the MSC_textual_rules ontology.

$report_file_location = "..\\ontology".$output_suffix."\\generate_genomic_CDS_report.txt";//Output report about problems that may happen during the execution of this script.


$first_part_ontology = "..\\ontology".$output_suffix."\\MSC_poly_allele".$output_suffix.".owl";
$second_part_ontology = "..\\ontology".$output_suffix."\\MSC_allele_rule".$output_suffix.".owl";
...
foreach ($xml->Rs as $Rs) {
	...
	*/
	...
	foreach ($observed_alleles as $observed_allele) {
		...
		$polymorphisms .= "Class: " . $variant_class_id . "\n";
		$polymorphisms .= "    SubClassOf: " . $class_id . "\n";
		
		if(strpos($observed_allele,trim($ref_allele)) !== false || $observed_allele == $ref_allele){
			$polymorphisms .= "    Annotations: vcf_format_reference \"true\" \n";
		}else{
		
		}
		$polymorphisms .= "    Annotations: rdfs:label \"" . $variant_class_id . "\" \n\n";
		$polymorphism_variant_disjoint_list[] = $variant_class_id;
		 
		$valid_polymorphism_variants[] = $variant_class_id;
	}	
	
	$polymorphisms .= generateDisjointClassesOWL($polymorphism_variant_disjoint_list);
...
foreach ($gene_ids as $gene_id) {
	$allele_classes .= "Class: " . $gene_id . "\n";
	$allele_classes .= "    SubClassOf: allele \n";
	$allele_classes .= "    Annotations: rdfs:label \"" . $gene_id . "\" \n";
	$allele_classes .= "	 Annotations: rank \"" . $genotype_rank . "\" \n\n";
	$genotype_rank++;
}
...
foreach ($objPHPExcel->getWorksheetIterator() as $objWorksheet) {// We analyze every haplotype in the document.
	$homozygous_haplotype_id_array = array();
	...
	foreach ($objWorksheet->getRowIterator() as $row) {
		...
		//
		//$gene_label_cut = substr($gene_label,strpos($gene_label," "));
		//$human_gene_label = "human with " . $gene_label_cut;
		//$human_gene_id = make_valid_id($human_gene_label);
		//$owl .= "Class: " . $human_gene_id . "\n";
		//$owl .= "SubClassOf: human_with_genetic_polymorphism" . "\n";
		//$owl .= "Annotations: rdfs:label \"" . $human_gene_label . "\"\n";
		//
		...
		if (!isset($row_array['C']) || $row_array['C'] == "") {// If cell in superclass column is empty...
			...
			$allele_classes_eq .= "Class: " . $allele_id . "\n";
			$allele_classes_eq .= "    Annotations: rdfs:label \"" . $allele_label . "\"\n";
			$allele_classes_eq .= "    SubClassOf: " . $gene_id . "\n\n";
			
			$allele_classes_su .= "Class: " . $allele_id . "\n";
			$allele_classes_su .= "    Annotations: rdfs:label \"" . $allele_label . "\"\n";
			$allele_classes_su .= "    SubClassOf: " . $gene_id . "\n\n";
			...
			if($gene_id == "CYP2D6"){
				...
				$allele_classes_eq .= "Class: " . $duplicated_allele_id . "\n";
				$allele_classes_eq .= "    Annotations: rdfs:label \"" . $duplicated_allele_label . "\"\n";
				$allele_classes_eq .= "    SubClassOf: " . $gene_id . "\n\n";
				
				$allele_classes_su .= "Class: " . $duplicated_allele_id . "\n";
				$allele_classes_su .= "    Annotations: rdfs:label \"" . $duplicated_allele_label . "\"\n";
				$allele_classes_su .= "    SubClassOf: " . $gene_id . "\n\n";
			}
			...
			$allele_classes_eq .= "Class: " . $superclass_id . "\n";
			$allele_classes_eq .= "    Annotations: rdfs:label \"" . $superclass_label . "\"\n";
			$allele_classes_eq .= "    SubClassOf: " . $gene_id . "\n\n";
			
			$allele_classes_eq .= "Class: " . $allele_id . "\n";
			$allele_classes_eq .= "    Annotations: rdfs:label \"" . $allele_label . "\"\n";
			$allele_classes_eq .= "    SubClassOf: " . $superclass_id . "\n\n";
			
			$allele_classes_su .= "Class: " . $superclass_id . "\n";
			$allele_classes_su .= "    Annotations: rdfs:label \"" . $superclass_label . "\"\n";
			$allele_classes_su .= "    SubClassOf: " . $gene_id . "\n\n";
			
			$allele_classes_su .= "Class: " . $allele_id . "\n";
			$allele_classes_su .= "    Annotations: rdfs:label \"" . $allele_label . "\"\n";
			$allele_classes_su .= "    SubClassOf: " . $superclass_id . "\n\n";
		...
		if($gene_id == "CYP2D6"){
			...
			$allele_classes_su .= "Class: human_with_".$duplicated_allele_id . "\n";
			$allele_classes_su .= "SubClassOf: human_with_genetic_polymorphism,\n";
			$allele_classes_su .= "has some " . $duplicated_allele_id . "\n";
			$allele_classes_su .= "Annotations: rdfs:label \"human with " . $duplicated_allele_label . "\" \n\n";
			
			
			$allele_classes_eq .= "Class: ".$duplicated_allele_id . "\n";
			$allele_classes_eq .= "SubClassOf: \n";
			$allele_classes_eq .= $gene_id . "\n";
			
			$owl .= "Class: human_with_" . $duplicated_allele_id . "\n";
			$owl .= "SubClassOf: human_with_genetic_polymorphism" . "\n";
			//$owl .= "SubClassOf: " . $human_gene_id . "\n";
			$owl .= "SubClassOf:" . "\n";
		...
		$allele_classes_su .= "Class: " . $human_id . "\n";
		$allele_classes_su .= "SubClassOf: human_with_genetic_polymorphism" . "\n";
		$allele_classes_su .= "Annotations: rdfs:label \"" . $human_label . "\"\n";
		...
		if (!isset($row_array['A']) || $row_array['A'] != "disabled") {
			...
			if (empty($allele_polymorphism_variants) == false) {
				$allele_classes_eq .= "Class: ".$allele_id . "\n";
				$allele_classes_eq .= "EquivalentTo: \n";
				$allele_classes_eq .= "has some " . implode(" and has some ", $allele_polymorphism_variants);
				$allele_classes_eq .= "\n\n";
				
				$owl .= "EquivalentTo:" . "\n";
				$owl .= "human and has some " . implode(" and has some ", $allele_polymorphism_variants);
				$owl .= "\n\n";
			}
			else {
		...
		$allele_classes_su .= "Class: ".$human_id . "\n";
		$allele_classes_su .= "SubClassOf: \n";
		$allele_classes_su .= "has some " . $allele_id . "\n\n";
		
		if($gene_id == "CYP2D6"){
			$duplicated_allele_id = str_replace("CYP2D6","CYP2D6_duplicated",$allele_id);
			$duplicated_allele_label = str_replace("CYP2D6","CYP2D6 duplicated",$allele_label);
			$homozygous_haplotype_id_array[] = "homozygous_".$duplicated_allele_id;
			
			$allele_classes_su .= "Class: homozygous_" . $duplicated_allele_id . "\n";
			$allele_classes_su .= "SubClassOf: \n";
			$allele_classes_su .= $gene_id ."\n";
			
			$allele_classes_su .= "Class: human_with_homozygous_".$duplicated_allele_id . "\n";
			$allele_classes_su .= "SubClassOf: human_with_genetic_polymorphism,\n";
			$allele_classes_su .= "has exactly 2 " . $duplicated_allele_id . "\n\n";
			
			$allele_classes_eq .= "Class: homozygous_" . $duplicated_allele_id . "\n";
			$allele_classes_eq .= "SubClassOf: \n";
			$allele_classes_eq .= $gene_id ."\n";
		...
		$homozygous_haplotype_id_array[] = "homozygous_".$allele_id;
		$allele_classes_su .= "Class: homozygous_".$allele_id . "\n";
		$allele_classes_su .= "SubClassOf: \n";
		$allele_classes_su .= $gene_id ."\n";
		
		$allele_classes_su .= "Class: ".$human_homozygous_id."\n";
		$allele_classes_su .= "SubClassOf: human_with_genetic_polymorphism,\n";
		$allele_classes_su .= "Annotations: rdfs:label \"" . $human_homozygous_label . "\" \n";
		$allele_classes_su .= "has exactly 2 " . $allele_id . "\n\n";
				
		if (!isset($row_array['A']) || $row_array['A'] ==! "disabled") {
			// If there are tagging polymorphism variants...
			if (empty($allele_polymorphism_variants) == false) {
				$allele_classes_eq .= "Class: homozygous_".$allele_id . "\n";
				$allele_classes_eq .= "SubClassOf: ". $gene_id ."\n";
				$allele_classes_eq .= "EquivalentTo: \n";
				$allele_classes_eq .= "has exactly 2 " . implode(" and has exactly 2 ", $allele_polymorphism_variants);
				$allele_classes_eq .= "\n\n";
				
				$owl .= "EquivalentTo:" . "\n";
				$owl .= "human and has exactly 2 " . implode(" and has exactly 2 ", $allele_polymorphism_variants);
				$owl .= "\n\n";
			}
			$owl .= "SubClassOf:" . "\n";
	...
	foreach($haplotypes_disjoints as $group_alleles_disjoints){
		//$allele_classes_su .= generateDisjointClassesOWL($group_alleles_disjoints);
		//$allele_classes_eq .= generateDisjointClassesOWL($group_alleles_disjoints);
	...
	$owl .= generateDisjointClassesOWL($homozygous_haplotype_id_array);
	//$allele_classes_su .= generateDisjointClassesOWL($group_alleles_disjoints)."\n";
	//$allele_classes_eq .= generateDisjointClassesOWL($group_alleles_disjoints)."\n";
}
...
foreach ($drug_labels as $drug_label) {
	$drug_classes .= "Class: ". make_valid_id($drug_label) . "\n";
	$drug_classes .= "    Annotations: rdfs:label \"" . $drug_label . "\"\n";
	$drug_classes .= "    SubClassOf: drug" . "\n\n";
...
$owl .= "# drug disjoints\n";
$drug_classes .= generateDisjointClassesOWL($drug_labels);
...
foreach ($objWorksheet->getRowIterator() as $row) {
	...
	$drug_classes .= "Class: " . make_valid_id($entity_label) . "\n";
	$drug_classes .= "    Annotations: rdfs:label \"" . $entity_label . "\"\n";
	$drug_classes .= parse_multiple_URLs($external_URL);
	$drug_classes .= "    Annotations: rdfs:comment \"" . $comment . "\"\n\n";
...
$polymorphisms .= generateDisjointClassesOWL($polymorphism_disjoint_list);
$owl .= generateDisjointClassesOWL($polymorphism_disjoint_list);
$owl .= "# gene/allele disjoints\n";
$allele_classes .= generateDisjointClassesOWL($gene_ids);
...
$root_content = file_get_contents($pharmacogenomic_CDS_base_file_location) . "\n\n\n";
$root_infer = file_get_contents($MSC_classes_base_file_location) . "\n\n\n";
file_put_contents($first_part_ontology, $root_content . $root_infer . $polymorphisms . $allele_classes . $allele_classes_eq . $drug_classes . $textual_rule);
file_put_contents($second_part_ontology, $root_content . $root_infer . $polymorphisms . $allele_classes . $allele_classes_su . $drug_classes . $rule_owl);
