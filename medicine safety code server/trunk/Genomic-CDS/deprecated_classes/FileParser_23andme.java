...
public class FileParser_23andme {
	 ...
	 * */
	public FileParser_23andme(ArrayList<SNPelement_old> listRsids,HashMap<String,String> criteriaSyntax2bitcode){
	...
	public String parse(InputStream my23AndMeFileStream, String my23AndMeStrandOrientation) {
		...
		try{
			...
			while((line=br.readLine())!=null){
				...
				for(int i=0;i<listRsids.size();i++){
					...
					if(genotype.getRsid().equalsIgnoreCase(rsid)){//Check which is the related marker defined in the model for this strand
						processedMatchingLines++;
						String[] variants = getVariants(my23AndMeSNPCode,my23AndMeStrandOrientation,genotype.getOrientation()); //Obtain the correct code regarding orientation and alphabetical order.
						genotype.setBit_code(rsid+"("+variants[0]+";"+variants[1]+")");//Generate the new criteria syntax for this strand
					}
				}
			}
			br.close();
		...
		for(int i=0;i<listRsids.size();i++){
			SNPelement_old genotype = listRsids.get(i);
			String criteriaSyntax=genotype.getBit_code();//Criteria syntax that will be related to the patient
