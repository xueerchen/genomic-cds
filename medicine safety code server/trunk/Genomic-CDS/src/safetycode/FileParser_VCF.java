...
public class FileParser_VCF implements FileParser {
	...
	public FileParser_VCF(ArrayList<String[]> listRsids, HashMap<String, String> criteriaSyntax2bitcode) {
	...
	public String parse(InputStream fileStream, String strandOrientation) {
			...
			while((line=br.readLine())!=null){
				...
				if(listRsids!=null){
					for(int i = 0; i < listRsids.size(); i++){
						...
						if(genotype[0].equals(rsid)){
							processedMatchingLines++;
							
							String[] variants = getVariants(snpCode,strandOrientation,genotype[2]); //Obtain the correct code regarding orientation and alphabetical order.
							...
							if(criteriaSyntax2bitcode!=null){
								...
								if(criteriaSyntax2bitcode.containsKey(criteriaSyntax)){
									...
									genotype[4]=bit_code;
									genotype[3]=criteriaSyntax;
								}else{
									genotype[3] = rsid+"(null;null)";
									genotype[4] = criteriaSyntax2bitcode.get(genotype[3]);
		...
		try{
			...
			for(int i=0;i<listRsids.size();i++){
				base2ProfileString+=listRsids.get(i)[4];			
			}
